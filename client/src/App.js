...
const App = () => {
  const [Web3States, setWeb3States] = useState({storageValue: 0, web3: null, accounts: null, contractInst: null});
  ...
  const getAndSetWeb3 = async () => {
    try {
      ...
      const instance = new w3.eth.Contract(SimpleStorageContract.abi, deployedNetwork && deployedNetwork.address);
  ...
  return (
    <div className="App">
      ...
      <div>The stored value is: {Web3States.storageValue}</div>
    </div>
  );
